/*
 * Topic Manager Content Script
 * -----------------------------------
 * Manages the sidebar, topics, bookmarks, and chat title for the extension.
 ...
 *   2. Utility Functions
 *   3. Storage Utilities
 *   4. DOM Extraction & Observers
 *   5. Sidebar & UI Management
 *   6. Topic Management
 *   7. Bookmark Management
 *   8. Chat Title Management
 *   9. Initialization
 */
...
/**
 * Extract messages from the DOM and update internal state
 */
...
/**
 * Observe chat container for DOM changes (new messages, etc.)
 */
...
/**
 * Ensure sidebar exists and is set up
 */
...
/**
 * Update the sidebar: title, topics, and bookmarks
 */
function updateSidebar(retries = 0) {
    ...
    if (conversationId && messageNodes.length === 0 && retries < CONFIG.RETRY_LIMIT) {
...
/**
 * Set up event listeners for sidebar interactions (expand/collapse, theme, close, etc.)
...
/**
 * Make the sidebar draggable by mouse or touch.
 * @param {HTMLElement} element - The sidebar DOM element
 */
function makeDraggable(element) {
    let isDragging = false;
    let currentX, currentY, initialX, initialY, savePositionTimeout;
        ...
        StorageUtils.getCurrentTopicByConversation((currentTopicByConversation) => {
            ...
            StorageUtils.getBookmarksByConversation((allBookmarks) => {
                ...
                if (noTopicBookmarks.length) {
                    html += `
                        <div class="topic-item no-topic${currentTopicId === null ? ' current' : ''}" data-topic-id="none">
                            <span class="topic-name">Unassigned</span>
                            <span class="bookmark-count">${noTopicBookmarks.length}</span>
                        </div>
                    `;
                ...
                topics.forEach(topic => {
                    const topicBookmarksList = topicBookmarks[topic.id] || [];
                    html += `
                        <div class="topic-wrapper">
                            <div class="topic-item${topic.id === currentTopicId ? ' current' : ''}" data-topic-id="${topic.id}">
                                ...
                                <span class="bookmark-count">${topicBookmarksList.length}</span>
...
/**
 * Get the chat title from the DOM
 */
...
/**
 * Update the sidebar's chat title based on the current conversation.
 */
function updateSidebarTitle() {
    ...
    StorageUtils.getCustomChatTitles((titles) => {
        const newTitle = titles[conversationId] || getChatTitleFromDOM();
...
/**
 * Show the input for editing the chat title in the sidebar.
 */
...
/**
 * Save the edited chat title and update storage and UI.
 */
function saveTitleEdit() {
    ...
    if (titleSpan && editSpan && input) {
        ...
        StorageUtils.setCustomChatTitle(conversationId, newTitle, () => {
            titleSpan.textContent = newTitle;
            lastCustomTitleConversationId = conversationId;
            lastCustomTitleValue = newTitle;
...
/**
 * Cancel editing the chat title and restore the previous state.
 */
...
/**
 * Render a list of custom titles for navigation
 */
 ...
 */
const debouncedExtractMessages = debounce(() => extractMessagesFromDOM(), CONFIG.DEBOUNCE_DELAY_MS);
...
/**
 * Handle initial chat load: set up sidebar, title, topics, and bookmarks for the current chat
 */
function handleInitialChatLoad() {
    extractMessagesFromDOM();
    ensureSidebar();
    updateSidebar();
}
...
/**
 * Update current chat state and notify background script
 */
...
/**
 * Observe title changes in the DOM and trigger a full sidebar update
 */
function observeTitleChanges() {
    const titleObserver = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (mutation.type === 'childList' || mutation.type === 'characterData') {
                // Update the title and trigger a full sidebar refresh
                updateSidebarTitle();
    ...
    CONFIG.SELECTORS.TITLE.forEach(selector => {
        ...
        if (element) {
            titleObserver.observe(element, { childList: true, characterData: true, subtree: true });
        }
    });

    const firstUserMessage = document.querySelector('[data-message-author-role="user"]');
    if (firstUserMessage) {
        titleObserver.observe(firstUserMessage, { childList: true, characterData: true, subtree: true });
    }
}

/**
 * Observe clicks on the ChatGPT sidebar
 */
function observeChatGPTSidebar() {
    ...
    chatHistory.addEventListener('click', (e) => {
        ...
        if (chatItem) {
            console.debug('[Topic Manager] Chat item clicked, updating title and sidebar');
            setTimeout(() => {
                extractMessagesFromDOM();
                updateSidebar();
            }, 200);
...
/**
 * Observe sidebar selection changes
 */
...
/**
 * Initialize the extension and set up observers and UI
 */
function init() {
    console.log('[Topic Manager] Initializing...');
    handleInitialChatLoad();
    observeUrlChanges();
    observeTitleChanges();
